...
$query = "SELECT o.order_id, concat(c.customer_firstName,' ',c.customer_lastName) as customer_name, o.shipper_id, o.shipper_date, o.shipper_address, o.order_quantity,  concat('Rp. ',format(o.order_grandtotal,0)) as 'order_grandtotal', o.order_updated FROM `order` o INNER JOIN `customers` c  ON o.customer_id = c.customer_id ORDER BY o.shipper_date DESC;";
